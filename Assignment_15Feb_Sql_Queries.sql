...
SELECT orderid,SUM((unitprice*quantity)-(unitprice*quantity*discount)) subtotal
FROM orderdetails 
GROUP BY orderid 
ORDER BY orderid;
...
SELECT o.shippeddate,ord.orderid,SUM((unitprice*quantity)-(unitprice*quantity*discount)) subtotal,EXTRACT(YEAR FROM orderdate) year
FROM orderdetails  ord
INNER JOIN orders o
ON o.orderid=ord.orderid
GROUP BY ord.orderid ,EXTRACT(YEAR FROM orderdate),o.shippeddate
ORDER BY orderid;
...
ORDER BY e.country,e.lastname,e.firstname,o.shippeddate,o.orderid;
...
SELECT o.orderid,p.productid,p.productname,o.unitprice,o.quantity,o.discount,SUM((unitprice*quantity)-(unitprice*quantity*discount)) subtotal
FROM orderdetails o
...
-- 14 - 1 Displays products(productname,unitprice) who’s price is greater than avg(price)
...
-- 14 - 2 Display product(productname), customers(companyname), orders(orderyear)
...
-- 14 - 3 Display Supplier Continent wise sum of unitinstock.
...
-- 14 - 4 Display top 10 expensive products
...
-- 14 - 5 Display customer supplier by city
